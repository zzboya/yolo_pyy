...
def soft_nms(bounding_boxes, confidence_score, threshold, label_boxes):
    ...
    while order.size > 0:
        ...
        order = order[left]
    return picked_boxes, picked_score, picked_label, sg
# 裁剪小图的路径
data_root = Path(r'E:\model\yoloair-main\yoloair-main\images_patch\DongYe05')
# label_root=Path(r'E:\model\yoloair-main\yoloair-main\runs\detect\exp66\labelsvoc')
label_root=Path(r'E:\model\yoloair-main\yoloair-main\images_label\DongYe05\labelsvoc')
tag = '\\'  # 路径分隔符
def custom_sort(x, y):
...
picked_boxes, picked_score,picked_label,sg = soft_nms(box, confidence_score = scores, threshold = iou_threshold,label_boxes=label_all)
print(picked_boxes)
...
# imc=io.imread(r'E:\model\yoloair-main\yoloair-main\images_big\ShiSheng01.jpg')
...
def xywh2xyxy(label,x,y,w,h, w1, h1, img,score):
    ...
    # cv2.imshow('show', img)
    cv2.imwrite(r'E:\model\yoloair-main\yoloair-main\images_big\DongYe05\output_iousugar211.png',img)


    cv2.waitKey(0)  # 按键结束
    cv2.destroyAllWindows()
img = cv2.imread(str(r'E:\model\yoloair-main\yoloair-main\images_big\DongYe05\DongYe05.jpg'))
h1, w1 = img.shape[:2]
bbox_index = 0
with open(r'E:\model\yoloair-main\yoloair-main\images_big\DongYe05\DongYe051.txt', "w+") as f:
    data = f.read()

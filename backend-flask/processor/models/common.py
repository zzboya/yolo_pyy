...
from processor.utils.datasets import exif_transpose, letterbox
from processor.utils.general import (LOGGER, check_requirements, check_suffix, check_version, colorstr, increment_path,
                           make_divisible, non_max_suppression, scale_coords, xywh2xyxy, xyxy2xywh)
from processor.utils.plots import Annotator, colors, save_one_box
from processor.utils.torch_utils import copy_attr, time_sync

from processor.models.module import *
...
from processor.models.Models.mobileone import MobileOneBlock
...
class BiFPN_Add6(nn.Module):
    def __init__(self, c1, c2):
        super(BiFPN_Add6, self).__init__()
    ...
    def forward(self, x):
        ...
        weight = w / (torch.sum(w, dim=0) + self.epsilon)
        # Fast normalized fusion
        return self.conv(self.silu(weight[0] * x[0] + weight[1] * x[1] + weight[2] * x[2] + weight[3] * x[3] + weight[4] * x[4] + weight[5] * x[5]))
...
class Concat_bifpn(nn.Module):
    ...
    def __init__(self, c1, c2):
        ...
        self.w6_weight = nn.Parameter(torch.ones(6, dtype=torch.float32), requires_grad=True)
        self.epsilon = 0.0001
    ...
    def forward(self, x): # mutil-layer 1-3 layers #ADD or Concat
        ...
        if len(x) == 2:
            ...
            w = self.w6_weight
            weight = w / (torch.sum(w, dim=0) + self.epsilon)
            x = self.conv(self.act(weight[0] * x[0] + weight[1] * x[1] + weight[2] *x[2] + weight[3]*x[3] + weight[4] * x[4] + weight[5] * x[5]))
        return x
